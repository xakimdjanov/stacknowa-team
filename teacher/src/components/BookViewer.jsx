...
/**
 * BookViewer - O'qituvchi va Talabalar uchun amaliy ishni kitob (booklet) shaklida ko'rish komponenti
 * 
...
const BookViewer = ({ pages = [], documentTitle = "Amaliy Mashg'ulot Hisoboti", studentName = "" }) => {
  const [viewMode, setViewMode] = useState('booklet'); // 'booklet' | 'single' | 'continuous'
          ...
          <div className="flex items-center space-x-2">
            ...
            <div className="flex items-center space-x-1 text-xs">
              <select
                ...
                className="bg-slate-900 border border-slate-800 text-slate-200 text-xs rounded-xl px-2.5 py-1.5 focus:outline-none focus:border-blue-500 cursor-pointer font-medium"
              >
                {safePages.map((p, idx) => (
                ...
                return (
                  <button
                    ...
                    className={`w-full text-left p-2.5 rounded-xl border transition-all flex flex-col space-y-1.5 ${
                      isActive 
                        ? 'bg-blue-950/60 border-blue-500 text-white shadow-md ring-1 ring-blue-500/40' 
                        : 'bg-slate-900/60 border-slate-800 text-slate-400 hover:bg-slate-800 hover:text-slate-200'
                    }`}
                  >
                    <div className="flex items-center justify-between text-[11px] font-bold">
                      <span className="truncate pr-1">{idx + 1}-bet: {page.title || `Sahifa ${idx + 1}`}</span>
                      {isActive && <span className="text-[10px] text-blue-400 font-extrabold flex-shrink-0">• Faol</span>}
                    </div>
                    ...
                    <div className="relative group">
                      ...
                      {totalPages > 1 && (
                        <button
                          type="button"
                          onClick={goNext}
                          className="absolute -right-5 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-blue-600 hover:bg-blue-700 text-white shadow-xl flex items-center justify-center transition-transform hover:scale-110 z-10"
                  ...
                  <div className="flex flex-col items-center">
                    <div className="relative flex items-stretch select-text">
                      <button
                        type="button"
                        onClick={goPrev}
                        className="absolute -left-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-slate-800/90 hover:bg-blue-600 text-white shadow-xl border border-slate-700 flex items-center justify-center transition-all hover:scale-110 z-20"
                      ...
                      <button
                        ...
                        className="absolute -right-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-slate-800/90 hover:bg-blue-600 text-white shadow-xl border border-slate-700 flex items-center justify-center transition-all hover:scale-110 disabled:opacity-0 disabled:pointer-events-none z-20"
                ...
                <div className="relative">
                  {currentPageIndex > 0 && (
                    <button
                      type="button"
                      onClick={goPrev}
                      className="absolute -left-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-slate-800 hover:bg-blue-600 text-white shadow-xl border border-slate-700 flex items-center justify-center transition-all hover:scale-110 z-20"
                      title="Oldingi bet"
                    >
                      <ChevronLeft className="w-6 h-6" />
                    </button>
                  ...
                  {currentPageIndex < totalPages - 1 && (
                    <button
                      type="button"
                      onClick={goNext}
                      className="absolute -right-6 top-1/2 -translate-y-1/2 w-12 h-12 rounded-full bg-slate-800 hover:bg-blue-600 text-white shadow-xl border border-slate-700 flex items-center justify-center transition-all hover:scale-110 z-20"
                      title="Keyingi bet"
                    >
                      <ChevronRight className="w-6 h-6" />
                    </button>
